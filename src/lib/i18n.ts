...
const messages = {
  'zh-CN': {
    ...
    'output.timeout': '\n[运行超时: {time}ms]',
    'output.outputLimit': '输出过多，已自动停止程序',
    'output.exitInfo': '\n[退出代码: {code}, 耗时: {time}ms]',
    'status.ready': '就绪',
    'status.unsavedPrefix': '未保存',
  ...
  'en-US': {
    ...
    'output.timeout': '\n[Timeout: {time}ms]',
    'output.outputLimit': 'Output limit exceeded; program stopped',
    'output.exitInfo': '\n[Exit Code: {code}, Time: {time}ms]',
    'status.ready': 'Ready',
    'status.unsavedPrefix': 'Unsaved',
    ...
    'status.outputLimit': 'Output limit exceeded',
