...
- 修复 Windows 自动打包时 MinGW 被解压成 `mingw64/mingw64` 双层目录的问题，新安装包恢复为单层 `mingw64` 工具链目录。
- 恢复默认设置时优先选择安装目录 `_up_/tools/mingw64/bin/g++.exe` 中的内置编译器，不再因路径检测失败退化为裸 `g++`。
- 兼容 v1.5.1 已安装版本的双层 MinGW 目录，升级后仍可自动识别原有内置编译器。
...
- 修复自动补全按 Enter 会接受建议而非换行的问题；现在 Enter 键保持原有代码编辑行为（换行 / 智能换行），接受建议请使用 Tab 或鼠标。

### v1.0.3

- 编辑器/输出面板分割比例和 IO 面板可见性现在会持久化保存，下次启动自动恢复。
- 输入/输出区字号现在跟随编辑器字号设置，修改字号后同步生效。
- 修复设置弹窗在缩放后显示比例异常的问题。
- 修复缩放后界面布局可能超出视口的问题。
...
看到桌面窗口弹出，且标题显示 `33IDE Lite v1.5.2`，说明本地开发环境已经正常启动。
